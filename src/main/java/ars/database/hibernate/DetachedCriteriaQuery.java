...
public class DetachedCriteriaQuery<T> implements Query<T> {
     ...
     * 获取属性查询描述
     *
     * @param property 属性名称，可使用“.”号隔开
     * @param joinType 外键关联类型
     * @return 描述名称
     */
    ...
    public List<?> stats() {
        if (!this.loaded) {
            ...
            if (this.projections.getLength() == 0) {
                throw new IllegalStateException("Projections must not be empty");
            }
            Session session = this.sessionFactory.openSession();
            try {
